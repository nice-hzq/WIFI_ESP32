...
| 状态 | 开始采集 | 停止采集 | 新建会话 | 运行分析 | 清除结果 | 开始测量 | 停止测量 |
|---|---|---|---|---|---|---|---|
| IDLE | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ | ❌ |
| COLLECTING | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| STOPPING | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| `joint_status` | 关节线程→UI | state, message | 更新 joint_status_var / 标定状态 |
...
| 近端/远端显示 | `tk.Label` ×2 | `self._joint_prox_var` / `self._joint_dist_var` | 自动更新 |
