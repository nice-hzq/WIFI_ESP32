...
| `device_detected` | `device_id`, `alias` | 检测到新设备上线 | 重新加载 device_map 并刷新近端/远端设备 ID 显示 |
| `identified` | `proximal_id`, `distal_id`, `proximal_alias`, `distal_alias` | 近端/远端传感器识别完成 | 用结构化字段直接更新设备 ID 显示 |
...
| 近端/远端显示 | `tk.Label` ×4 | `_joint_prox_var` / `_joint_dist_var` / `_joint_prox_device_var` / `_joint_dist_device_var` | 别名 + `→` + 设备 ID (WT****) |
...
**🆕 设备映射机制**:
- 近端/远端显示格式: `L4 → WT901AB12345`（别名 + 箭头 + 设备 ID）
- 无映射时设备 ID 显示 `—`
